...
def main():
    ...
    x_list = []  # if the video is existed
    y_list = []
    pre_x = 0
    pre_y = 0

    while(cap.isOpened()):
        grabbed, frame = cap.read()  # read the frame from the video
        if frame is None:
            break
